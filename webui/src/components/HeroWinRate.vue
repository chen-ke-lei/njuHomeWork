<template>
  <div id="heroWinRate">
    ...
    <br />
    <el-button plain icon="el-icon-video-play" size="mini" @click="this.mock">
      start
    </el-button>
...
export default {
  ...
  methods: {
        ...
        for (let j = 0; j < this.echartsOption.series.length; j++) {
          ...
          for (let i = batch - 1; i >= 0; i--) {
            ...
            if (this.dataBuffer[i].hero === this.echartsOption.series[j].name) {
              ...
              if (this.dataBuffer[i].updateTime > latestTime) {
                ...
                //记录胜率
                this.echartsOption.series[j].data.push(
                  (this.dataBuffer[i].winRate * 100).toFixed(3)
                );
                hasChange = true;
                ...
                if (
                  ...
                  this.dataBuffer[i].winRate !=
                    this.echartsOption.series[j].data[
                      this.echartsOption.series[j].data.length - 1
                    ]
                ) {
                  //记录胜率
                  this.echartsOption.series[j].data.push(
          ...
          if (c === batch && hasChange) {
            ...
            //如果该英雄有数据，则取最近一次胜率数据，保持不变
              ...
              this.echartsOption.series[j].data.push(
                this.echartsOption.series[j].data[lastIndex - 1]
              );
            }
          }
        }
    ...
    test1() {
      ...
      this.yieldRate.push(
        (testData[Math.floor(Math.random() * 10)].winRate * 100).toFixed(3)
      );
